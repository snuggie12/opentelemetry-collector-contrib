...
The SRV Resolver is useful in situations when you want to return hostnames instead of IPs for endpoints. An example would be a `StatefulSet`-backed headless kubernetes `Service` with istio.
...
> [!IMPORTANT]
> Currently priority and weight are not supported features
